...
def getTrending():
	...
	for item in items:
	    ...
	    #Stargazers-1
	    stargazers_doc = i('a[href="/{title}/stargazers"]'.format(title=project_title))
	    #Stargazers-2
	    ...
	    if language_name_coler == None:
	    	...
	    	show_name_coler = language_name_coler.replace("background-color: ", "")
	    ...
	    article = doc('<article>').attr('style', 'border: 1px solid #aaa;width: 100%;display: flex;justify-content: center;')
	    # Create the <div> element inside <article>
	    div = doc('<div>').attr('style', 'width: 90%;')
	    # Create the <h2> element inside <div>
	    project_title_span = '<span style="font-weight: normal;">{title}</span>'.format(title=project_title)
	    h2 = doc('<h2>').attr('style', 'color: #167df5')
	    h2.append(project_title_span)
	    # Create the <p> element inside <div>
	    p = doc('<p>').attr('style', 'color: #262626').text(project_description)
	    # Create the first <h3> element inside <div>
	    div1_attribute_value = 'color: {language_bgc};'.format(language_bgc=show_name_coler)
	    div1 = doc('<h3>')
	    div1.attr('style', div1_attribute_value)
	    div1.text(show_name)
	    div2 = doc('<div>').attr('style', 'margin-top: 1px;')
	    div2_val = '<span>{Stargazers}</span>'.format(Stargazers=stargazers)
	    div2.append(div2_val)
	    # Create the third <div> element inside <div>
	    div3 = doc('<div>').attr('style', 'margin-top: 1px;')
	    div3_val = '<span>{starsToday}</span>'.format(starsToday=starsToday)
